...
assert(pages.length === 93, `complete workbook must contain exactly 93 student pages; found ${pages.length}`);
assert(new Set(pages).size === 93, 'page numbers must be unique');
pages.forEach((page, index) => assert(page === index + 1, `complete workbook must be continuous from 1 to 93; found page ${page} at position ${index + 1}`));
...
console.log(`Circle QA: PASS (93 student pages checked; continuous 1–93 incl. pages 91–93 symmetry/congruence/locus/coordinate-classification unit; reverse real-life circumference/area problems locked; no separate answer keys; canonical A4/no-name-date/coordinate rules locked)`);
