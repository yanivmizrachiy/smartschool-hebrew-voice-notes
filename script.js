...
// Updated: 20260510-095542
// Purpose:
// - stop flicker/reload loops
// - never update/reload while recording
// - no aggressive auto-refresh
// - update button appears only when update is waiting
// - preserve dictation, punctuation, clear, copy and premium UI
(function(){
  const VERSION = 'stable-recording-20260510-095542';
  ...
  async function stableUpdateCheck(reason) {
    if(!('serviceWorker' in navigator)) return;
    if(recording || document.body.classList.contains('listening')) return;

    const now = Date.now();
    if(updateCheckRunning) return;
    if(reason !== 'manual' && now - lastCheck < 90000) return;
    ...
    try {
      ...
      // Do not break the app if update check fails.
    } finally {
  ...
  window.checkForUpdates = function(){
    return stableUpdateCheck('manual');
  };
  ...
  window.installApp = async function(){
    ...
    if(!deferredPrompt) {
      showInstall();
      setStatus('אם ההתקנה לא מופיעה, פתח את תפריט הדפדפן ובחר הוסף למסך הבית.');
      return;
    }
  ...
  const originalStart = window.startDictation;
  window.startDictation = function(){
    try { localStorage.removeItem(FINAL_CLEAR_FLAG); } catch(e) {}
    hideUpdate();
    recording = true;
    setStatus('מקשיב...');
    if(typeof originalStart === 'function') return originalStart();
  };

  const originalStop = window.stopDictation;
  window.stopDictation = function(){
    recording = false;
    if(typeof originalStop === 'function') {
      const out = originalStop();
      setTimeout(function(){ stableUpdateCheck('after-stop'); }, 2500);
      return out;
    }
  };
  ...
  window.addEventListener('load', function(){
    ...
    setTimeout(function(){
      stableUpdateCheck('load');
    }, 3500);
  });

  document.addEventListener('visibilitychange', function(){
    if(!document.hidden && !recording) {
      setTimeout(function(){ stableUpdateCheck('visible'); }, 2500);
    }
  });

  window.addEventListener('online', function(){
    if(!recording) {
      setTimeout(function(){ stableUpdateCheck('online'); }, 2500);
    }
  });
})();
// <<< YANIV_STABLE_RECORDING_UPDATE_MANAGER <<<
