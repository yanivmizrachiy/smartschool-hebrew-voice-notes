import fs from 'node:fs';
import path from 'node:path';
import crypto from 'node:crypto';

const root = process.cwd();
const expected = {
  'visual-assets/anis-basics.png': {
    bytes: 2383847,
    width: 1055,
    height: 1491,
    sha256: 'b9cf1b2dd2ca1263a39fcfdef2b3cd295af0c8b1053bf818d249023863f5c65a'
  },
  'visual-assets/cone-in-my-head.png': {
    bytes: 2578227,
    width: 1055,
    height: 1491,
    sha256: 'ec95b439b498b8ef7191d226b09e4612cf332be3702cbd2c27d1f244b0d10b84'
  },
  'visual-assets/world-of-cones.png': {
    bytes: 3054080,
    width: 1055,
    height: 1491,
    sha256: 'ca46b93f94932b694d9d3bce301a1d5f885b458267d46a2c584359fd65247ee5'
  },
  'visual-assets/jerusalem-cone-vision.png': {
    bytes: 2817862,
    width: 1055,
    height: 1491,
    sha256: '78350a291425eadcf8b89ed18edff294afcb369498bca25edcef8bb62fae9bad'
  }
};

const PNG_SIGNATURE = Buffer.from([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a]);

function dimensions(buffer) {
  if (buffer.length < 33 || !buffer.subarray(0, 8).equals(PNG_SIGNATURE)) {
    throw new Error('invalid PNG signature');
  }
  if (buffer.readUInt32BE(8) !== 13 || buffer.toString('ascii', 12, 16) !== 'IHDR') {
    throw new Error('missing canonical IHDR');
  }
  return { width: buffer.readUInt32BE(16), height: buffer.readUInt32BE(20) };
}

const failures = [];
for (const [rel, lock] of Object.entries(expected)) {
  const file = path.join(root, rel);
  if (!fs.existsSync(file)) {
    failures.push(`${rel}: missing exact final source PNG`);
    continue;
  }
  const data = fs.readFileSync(file);
  let size;
  try {
    size = dimensions(data);
  } catch (error) {
    failures.push(`${rel}: ${error.message}`);
    continue;
  }
  const digest = crypto.createHash('sha256').update(data).digest('hex');
  if (data.length !== lock.bytes) failures.push(`${rel}: bytes ${data.length} != ${lock.bytes}`);
  if (size.width !== lock.width || size.height !== lock.height) failures.push(`${rel}: dimensions ${size.width}x${size.height} != ${lock.width}x${lock.height}`);
  if (digest !== lock.sha256) failures.push(`${rel}: sha256 ${digest} != ${lock.sha256}`);
}

if (failures.length) {
  console.error(`Final cone PNG verification failed with ${failures.length} issue(s):`);
  failures.forEach(item => console.error(`- ${item}`));
  process.exit(1);
}

console.log('OK: all four final cone PNG assets match the exact verified source bytes, PNG/IHDR dimensions and SHA-256 locks.');
