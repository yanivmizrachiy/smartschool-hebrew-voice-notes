...
`content/catalog.json` הוא קטלוג היישום של שלוש החוברות ומפנה ל־manifest של כל חוברת. הספירות והנתיבים של מעגל וגליל נשמרים ב־`content/circle.json` וב־`content/cylinder.json`; חרוט נשאר ב־`content/workbook.json`. קובצי `content/schemas/` והבדיקה `tests/catalog-contract.mjs` מאמתים את מבנה הנתונים ואת הסכום 180.
...
- `npm run validate:dist` — בודק שה־deployment artifact מכיל את כל 180 הדפים ואינו מדליף קבצי פיתוח/QA.
